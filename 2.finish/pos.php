...
$result_check_record = $product->select($chm, $sbranch, $newcode, $psize);

if(mysql_num_rows($result_check_record)<1){

    $result_top_insert = $product->insert($pname,$psize,$newcode, $pqts,$sbranch,$chm);
    while($rows=mysql_fetch_array($result_tp_quantity)) {extract($rows);
    }
    // TODO
    ...
    $newqty=$topqty+$pqts;
    $result_update_tp = $product->update();
...
$profile = $clientProfile->select();
    $clientProfile->insert();
$profileData = $clientProfile->select();
